...
#
# The protected branch is DERIVED FROM REALITY: by default it is the repository's
# default branch (`gh repo view --json defaultBranchRef`), never a hardcoded
# "main". Set BRANCH=... to override explicitly.
#
...
# force-push the default branch, bypassing every rule above. The gate is absolute
# only for non-admin actors. Strict mode (no escape hatch) = remove
...
# Resolve the protected branch: explicit override wins, otherwise the repo's
# real default branch. Never hardcode a branch name.
